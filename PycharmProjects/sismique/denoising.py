...
if __name__=="__main__":
    ...
    plt.figure()
    dat_04 = genfromtxt('/Users/ferdi/PycharmProjects/sismique/data/Seismic/x04_2.txt', delimiter=',')
    dat_01 = genfromtxt('/Users/ferdi/PycharmProjects/sismique/data/Seismic/x01_2.txt', delimiter=',')
    ...
    print(len(sTV_hat[0, :]))
    d_gauss = np.zeros(len(sTV_hat[0, :]))
    d_TV = np.zeros(len(sTV_hat[0, :]))
    dTV_av = np.zeros(len(w))
    dG_av = np.zeros(len(w))
    for k in range(len(w)):
        for i in range(len(sTV_hat[0, :])):
            d_gauss[i] = np.linalg.norm(sG_hat - s_hat[i])
            d_TV[i] = np.linalg.norm(sTV_hat[k, :] - s_hat[i])
        dTV_av[k] = np.mean(d_TV)
        dG_av[k] = np.mean(d_gauss)
